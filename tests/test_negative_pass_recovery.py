from pages.auth import Registration, PassRecoveryPage
from selenium.webdriver.common.by import By
from config import valid_email, new_password
from settings import generate_string_rus, generate_string_en, english_chars, chinese_chars, \
    russian_chars, special_symbols
from pages.locators import PassRecoveryLocators, AuthLocators
import time
import pytest
...
def test_forgot_password_page(browser):
    ...
    result_id, status_id = Registration().get_id_letter(mail_name, mail_domain)
    ...
    result_code, status_code = Registration().get_reg_code(mail_name, mail_domain, str(id_letter))
    ...
    elem_new_pass = browser.find_element(PassRecoveryLocators.NEWPASS_NEW_PASS)
    elem_conf_pass = browser.find_element(PassRecoveryLocators.NEWPASS_NEW_PASS_CONFIRM)
    ...
    new_pass = f'{new_password}{generate_string_rus(1)}'
    input_new_pass(new_pass)
    ...
    for i in new_pass:
        if i.isdigit() or i in special_symbols():
    ...
    elem_new_pass.send_keys(Keys.COMMAND, 'a')
    elem_new_pass.send_keys(Keys.DELETE)
    new_pass = f'{new_password[0:8]}{generate_string_en(2)}'
    elem_new_pass.send_keys(new_pass)
    time.sleep(3)

    elem_conf_pass.send_keys(Keys.COMMAND, 'a')
    elem_conf_pass.send_keys(Keys.DELETE)
    new_conf_pass = f'{new_password[0:8]}{generate_string_en(4)}'
